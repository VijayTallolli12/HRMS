<x-app-layout>
    <x-page-header title="Edit Leave" description="Update leave request details." icon="calendar">
                ...
                <div class="space-y-5">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            ...
                            <input id="start_date" name="start_date" type="date" class="input-field" value="{{ old('start_date', $leave->start_date->format('Y-m-d')) }}" required />
                            <x-input-error :messages="$errors->get('start_date')" class="mt-2" />
                        </div>
                        <div>
                            <label for="end_date" class="label">End Date <span class="text-red-500">*</span></label>
                            <input id="end_date" name="end_date" type="date" class="input-field" value="{{ old('end_date', $leave->end_date->format('Y-m-d')) }}" required />
                            <x-input-error :messages="$errors->get('end_date')" class="mt-2" />
                        </div>
                    </div>
                    <div>
                        <label for="days" class="label">Days <span class="text-red-500">*</span></label>
                        <input id="days" name="days" type="number" class="input-field" value="{{ old('days', $leave->days) }}" min="1" required />
                        <x-input-error :messages="$errors->get('days')" class="mt-2" />
                    </div>
                    <div>
                        <label for="reason" class="label">Reason</label>
                        <textarea id="reason" name="reason" rows="3" class="textarea-field">{{ old('reason', $leave->reason) }}</textarea>
                        <x-input-error :messages="$errors->get('reason')" class="mt-2" />
                    </div>
                    <div class="flex items-center justify-end gap-3 mt-6 pt-6 border-t border-gray-100">
                        <a href="{{ route('leaves.show', $leave) }}" class="btn-ghost" wire:navigate>Cancel</a>
