<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <form action="{{ route('designations.index') }}" method="GET" class="flex gap-2 flex-wrap">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @if ($designations->isEmpty())
                    <x-empty-state title="No designations found" description="Get started by creating your first designation.">
                        @can('create-designation')
                            <a href="{{ route('designations.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700" wire:navigate>Create Designation</a>
                        @endcan
                    </x-empty-state>
                @else
                            ...
                            <tbody class="bg-white divide-y divide-gray-200">
                                @foreach ($designations as $desig)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{{ $desig->department->name ?? '-' }}</td>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            @can('update-designation')
                                                <a href="{{ route('designations.edit', $desig) }}" class="text-indigo-600 hover:text-indigo-900" wire:navigate>Edit</a>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
