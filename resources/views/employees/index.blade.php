<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6">
                <form action="{{ route('employees.index') }}" method="GET" class="flex gap-2 flex-wrap">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                @if ($employees->isEmpty())
                    <x-empty-state title="No employees found" description="Get started by adding your first employee.">
                        @can('create-employee')
                            <a href="{{ route('employees.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700" wire:navigate>Add Employee</a>
                        @endcan
                    </x-empty-state>
                @else
                                ...
                                @foreach ($employees as $emp)
                                    <tr>
                                        ...
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                                            @can('view-employee')
                                                <a href="{{ route('employees.show', $emp) }}" class="text-blue-600 hover:text-blue-900" wire:navigate>View</a>
                                            @endcan
                                            @can('update-employee')
                                                <a href="{{ route('employees.edit', $emp) }}" class="text-indigo-600 hover:text-indigo-900" wire:navigate>Edit</a>
                                            @endcan
                                        </td>
                                    </tr>
                                @endforeach
