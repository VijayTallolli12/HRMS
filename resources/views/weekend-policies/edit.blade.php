<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit {{ $weekendPolicy->name }}</h2>
    </x-slot>
    <div class="py-12">
                ...
                <form action="{{ route('weekend-policies.update', $weekendPolicy) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $weekendPolicy->name)" required />
                            ...
                            <div class="mt-2 flex flex-wrap gap-4">
                                @foreach (['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday', 'Sunday'] as $day)
                                    <label class="inline-flex items-center gap-2">
                                        <input type="checkbox" name="weekend_days[]" value="{{ $day }}" {{ in_array($day, old('weekend_days', $weekendPolicy->weekend_days ?? [])) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" />
                        ...
                        <div>
                            ...
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $weekendPolicy->description) }}</textarea>
                        ...
                        <div class="flex items-center justify-end gap-2">
                            ...
                            <x-primary-button type="submit">Update Weekend Policy</x-primary-button>
                        </div>
                    </div>
                </form>
            </div>
