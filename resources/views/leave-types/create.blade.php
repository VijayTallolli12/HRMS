<x-app-layout>
    <x-page-header title="Create Leave Type" description="Define a new leave category for employee time off." icon="tag">
            ...
            <form action="{{ route('leave-types.store') }}" method="POST">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input id="name" name="name" type="text" class="input-field" value="{{ old('name') }}" required autofocus />
                        @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="days_per_year" class="label">Days Per Year <span class="text-red-500">*</span></label>
                        <input id="days_per_year" name="days_per_year" type="number" class="input-field" value="{{ old('days_per_year') }}" required />
                        @error('days_per_year') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div class="flex items-end">
                        <label class="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-gray-300 text-primary-600 shadow-sm focus:ring-primary-500" />
