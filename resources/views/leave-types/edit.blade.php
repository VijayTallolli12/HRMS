<x-app-layout>
    <x-page-header title="Edit Leave Type" icon="heroicon-o-calendar">
        ...
        <form action="{{ route('leave-types.update', $leaveType) }}" method="POST">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <x-text-input id="days_per_year" name="days_per_year" type="number" class="mt-1 block w-full" :value="old('days_per_year', $leaveType->days_per_year)" required />
                    @error('days_per_year')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="flex items-end">
                    <label class="flex items-center gap-2">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $leaveType->is_active) ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" />
