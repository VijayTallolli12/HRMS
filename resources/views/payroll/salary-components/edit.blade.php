<x-app-layout>
    <x-slot name="header">
        <x-page-header
            ...
            icon="cube"
            description="Update the {{ $comp->name }} component settings."
        >
            <x-slot name="breadcrumb">
                <nav class="breadcrumb">
                    ...
                    <span class="text-gray-400 mx-2">/</span>
                    <span class="text-gray-900">Edit {{ $comp->name }}</span>
                </nav>
            </x-slot>
        </x-page-header>
    </x-slot>
    ...
    <div class="max-w-3xl mx-auto">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('payroll.salary-components.update', $comp) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="space-y-5">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <x-text-input id="name" name="name" type="text" class="mt-1.5 block w-full" :value="old('name', $comp->name)" required />
                                <x-input-error :messages="$errors->get('name')" class="mt-1.5" />
                            </div>
                            <div>
                                <x-input-label for="code" value="Code *" />
                                <x-text-input id="code" name="code" type="text" class="mt-1.5 block w-full" :value="old('code', $comp->code)" required />
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <select id="type" name="type" class="select-field mt-1.5 block w-full" required>
                                    ...
                                    <option value="earning" {{ old('type', $comp->type) === 'earning' ? 'selected' : '' }}>Earning</option>
                                    <option value="deduction" {{ old('type', $comp->type) === 'deduction' ? 'selected' : '' }}>Deduction</option>
                                    <option value="benefit" {{ old('type', $comp->type) === 'benefit' ? 'selected' : '' }}>Benefit</option>
                                </select>
                            ...
                            <div>
                                ...
                                <select id="calculation_type" name="calculation_type" class="select-field mt-1.5 block w-full" required>
                                    ...
                                    <option value="fixed" {{ old('calculation_type', $comp->calculation_type) === 'fixed' ? 'selected' : '' }}>Fixed</option>
                                    <option value="percentage" {{ old('calculation_type', $comp->calculation_type) === 'percentage' ? 'selected' : '' }}>Percentage</option>
                                </select>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div>
                                ...
                                <x-text-input id="default_value" name="default_value" type="number" step="0.01" class="mt-1.5 block w-full" :value="old('default_value', $comp->default_value)" required />
                                <x-input-error :messages="$errors->get('default_value')" class="mt-1.5" />
                            </div>
                            <div class="flex items-end">
                                <label class="flex items-center gap-3 cursor-pointer">
                                    <input type="checkbox" name="is_active" value="1" {{ old('is_active', $comp->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded border-gray-300 text-primary-600 focus:ring-primary-500" />
