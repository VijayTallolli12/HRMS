<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">Edit {{ $latePolicy->name }}</h2>
    </x-slot>
    <div class="py-12">
                ...
                <form action="{{ route('late-policies.update', $latePolicy) }}" method="POST">
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $latePolicy->name)" required />
                            <x-input-error :messages="$errors->get('name')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="grace_minutes" value="Grace Minutes" />
                            <x-text-input id="grace_minutes" name="grace_minutes" type="number" min="0" max="60" class="mt-1 block w-full" :value="old('grace_minutes', $latePolicy->grace_minutes)" required />
                            <x-input-error :messages="$errors->get('grace_minutes')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="max_late_per_month" value="Max Late Per Month" />
                            <x-text-input id="max_late_per_month" name="max_late_per_month" type="number" min="1" max="31" class="mt-1 block w-full" :value="old('max_late_per_month', $latePolicy->max_late_per_month)" required />
                            <x-input-error :messages="$errors->get('max_late_per_month')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="penalty_type" value="Penalty Type" />
                            <select id="penalty_type" name="penalty_type" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm" required x-data="{ penaltyType: '{{ old('penalty_type', $latePolicy->penalty_type) }}' }" x-model="penaltyType">
                        ...
                        <div x-data="{ penaltyType: '{{ old('penalty_type', $latePolicy->penalty_type) }}' }" x-show="penaltyType === 'deduction'" x-cloak>
                            ...
                            <x-text-input id="penalty_amount" name="penalty_amount" type="number" step="0.01" min="0" class="mt-1 block w-full" :value="old('penalty_amount', $latePolicy->penalty_amount)" />
                            <x-input-error :messages="$errors->get('penalty_amount')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="description" value="Description" />
                            <textarea id="description" name="description" rows="3" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">{{ old('description', $latePolicy->description) }}</textarea>
                        ...
                        <div class="flex items-center justify-end gap-2">
                            <a href="{{ route('late-policies.show', $latePolicy) }}" class="text-gray-600 hover:text-gray-900" wire:navigate>Cancel</a>
                            <x-primary-button type="submit">Update Late Policy</x-primary-button>
                        </div>
                    </div>
                </form>
            </div>
